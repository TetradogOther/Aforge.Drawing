...
namespace AForge.Math.Geometry
{
    ...
    public class SimpleShapeChecker
    {
        ...
        public bool IsCircle( List<IntPoint> edgePoints, out Point center, out float radius )
        {
            ...
            {
                meanDistance += (float) System.Math.Abs( center.DistanceTo( edgePoints[i] ) - radius );
            }
        ...
        public PolygonSubType CheckPolygonSubType( List<IntPoint> corners )
        {
            ...
            if ( corners.Count == 3 )
            {
                ...
                if ( ( System.Math.Abs( angle1 - 60 ) <= angleError ) &&
                     ( System.Math.Abs( angle2 - 60 ) <= angleError ) &&
                     ( System.Math.Abs( angle3 - 60 ) <= angleError ) )
                {
                ...
                else
                {
                    ...
                    if ( ( System.Math.Abs( angle1 - angle2 ) <= angleError ) ||
                         ( System.Math.Abs( angle2 - angle3 ) <= angleError ) ||
                         ( System.Math.Abs( angle3 - angle1 ) <= angleError ) )
                    {
                        subType = PolygonSubType.IsoscelesTriangle;
                    }

                    // check for rectangled triangle
                    if ( ( System.Math.Abs( angle1 - 90 ) <= angleError ) ||
                         ( System.Math.Abs( angle2 - 90 ) <= angleError ) ||
                         ( System.Math.Abs( angle3 - 90 ) <= angleError ) )
                    {
            ...
            else if ( corners.Count == 4 )
            {
                ...
                if ( angleBetween1stPair <= angleError )
                {
                    ...
                    if ( angleBetween2ndPair <= angleError )
                    {
                        ...
                        if ( System.Math.Abs( GeometryTools.GetAngleBetweenVectors( corners[1], corners[0], corners[2] ) - 90 ) <= angleError )
                        {
                        ...
                        if ( System.Math.Abs( side1Length - side2Length ) <= maxLengthDiff )
                        {
        ...
        public bool CheckIfPointsFitShape( List<IntPoint> edgePoints, List<IntPoint> corners )
        {
            ...
            for ( int i = 0, n = edgePoints.Count; i < n; i++ )
            {
                ...
                for ( int j = 0; j < cornersCount; j++ )
                {
                    ...
                    if ( !isVert[j] )
                    {
                        distance = (float) System.Math.Abs( ( k[j] * edgePoints[i].X + b[j] - edgePoints[i].Y ) / div[j] );
                    }
                    else
                    {
                        distance = System.Math.Abs( edgePoints[i].X - corners[j].X );
                    }
